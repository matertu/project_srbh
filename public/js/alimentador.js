  ...
  document
    ...
    .addEventListener("input", async function () {
      ...
      if (val.length >= 7 && val.length <= 8) {
        const est = await Estacoes.buscarPorCodAna(val);
        // Previne condição de corrida se o usuário digitou mais números enquanto aguardava
        if (this.value.replace(/\D/g, "") !== val) return;
